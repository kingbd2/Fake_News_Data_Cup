...
from local_modules.hunga_bunga.utilities import *
from local_modules.hunga_bunga.universal_params import *
...
linear_models_n_params_small = [
    (LinearRegression, normalize),
    (Ridge, {"alpha": alpha_small, "normalize": normalize}),
    (Lasso, {"alpha": alpha_small, "normalize": normalize}),
    (
        ElasticNet,
        {"alpha": alpha, "normalize": normalize, "l1_ratio": [0.1, 0.3, 0.5, 0.7, 0.9]},
    ),
    (Lars, {"normalize": normalize, "n_nonzero_coefs": [100, 300, 500, np.inf]}),
    (
        LassoLars,
    ...
    (
        OrthogonalMatchingPursuit,
        {"n_nonzero_coefs": [100, 300, 500, np.inf, None], "normalize": normalize},
    ),
    (
        BayesianRidge,
        {
    ...
    (PassiveAggressiveRegressor, {"C": C, "n_iter": n_iter}),
    (
        RANSACRegressor,
        {
            "min_samples": [0.1, 0.5, 0.9, None],
    ...
    (HuberRegressor, {"max_iter": max_iter, "alpha_small": alpha_small}),
    (KernelRidge, {"alpha": alpha_small, "degree": degree}),
]

svm_models_n_params_small = [
    (SVR, {"kernel": kernel, "degree": degree, "shrinking": shrinking}),
    (
        NuSVR,
...
def gen_reg_data(
    x_mu=10.0, x_sigma=1.0, num_samples=100, num_features=3, y_formula=sum, y_sigma=1.0
):
    x = np.random.normal(x_mu, x_sigma, (num_samples, num_features))
    y = np.apply_along_axis(y_formula, 1, x) + np.random.normal(
        0, y_sigma, (num_samples,)
    )
    return x, y
...
def run_all_regressors(
    ...
    all_params = (
        (linear_models_n_params_small if small else linear_models_n_params)
    ...
    return main_loop(
        all_params,
        StandardScaler().fit_transform(x) if normalize_x else x,
        y,
