...
neighbor_models_n_params = [
    (KMeans, {"algorithm": ["auto", "full", "elkan"], "init": ["k-means++", "random"]}),
    (
        KNeighborsClassifier,
        {
    ...
    (
        NearestCentroid,
        {"metric": neighbor_metric, "shrink_threshold": [1e-3, 1e-2, 0.1, 0.5, 0.9, 2]},
    ),
    (
        RadiusNeighborsClassifier,
        {
...
tree_models_n_params_small = [
    (
        RandomForestClassifier,
        {
            "max_features_small": max_features_small,
            "n_estimators_small": n_estimators_small,
            "min_samples_split": min_samples_split,
...
class HungaBungaClassifier(ClassifierMixin):
    ...
    def fit(self, x, y):
        self.model = run_all_classifiers(
            x,
            y,
