...
def main_loop(
    ...
    def cv_():
        return (
            cv_clf(x, y, test_size, n_splits, random_state, upsample)
            if isClassification
        ...
        try:
            ...
            if clf_Klass == KMeans:
                ...
                parameters["max_features"] = [
                    v
                    for v in parameters["max_features"]
                    if v is None or type(v) == str or v <= num_features
                ]
            clf_search = GridSearchCV(
                clf_Klass(), parameters, scoring, cv=cv_(), n_jobs=n_jobs
            )
            clf_search.fit(x, y)
            ...
            if brain:
                print(
                    ...
                    "time/clf: %0.3f seconds" % timespent,
                )
            if brain:
                print("best params:")
            if brain:
    ...
    if brain:
        print(
            tabulate(
                [[m.__class__.__name__, "%.3f" % s, "%.3f" % t] for m, s, t in res],
    ...
    if brain:
        print(
            "The winner is: %s with score %0.3f."
            % (winner.__class__.__name__, res[winner_ind][1])
        )
    return winner, res
